...
from pytorch.dataset_torch import read_corpus, split_given_size, get_vocab, apply_reduction, subsample_prob, cache_subsample_prob, get_sampled_sent, get_dynamic_window
from pytorch.after_training_torch import _negative_sampling_loss_torch, fixed_unigram_candidate_sampler, _full_loss_torch
...
def build_dataset(text, max_window, whitelist=[], min_freq=1, sampling_rate=1e-3, dynamic_window=True, batch_n_sentence=None, fixed_batch_size=False):
    """
    ...
    count_vocab = dict(sorted(count_vocab.items(),key=lambda item: item[1], reverse=True))

    #ord_vocab = {key: i for i, key in enumerate(sorted(count_vocab.keys())) }
    ord_vocab = {key: i for i, key in enumerate(count_vocab.keys())}
    ...
    for n_sent, sentence in enumerate(get_text(_text)):
        ...
        if contains_weat(sentence, whitelist):
          ...
          if (BATCH_SIZE == 'auto') and (BATCH_N_SENTENCE < tmp_batch_sentence):
              _data.append(_sent_batch)
              _sent_batch = []
              tmp_batch_sentence = 0
...
def build_dataset_gensim(text, vocab, unigram_counts, inv_vocab, max_window, whitelist=[], min_freq=1, sampling_rate=1e-3, dynamic_window=True, batch_n_sentence=None, fixed_batch_size=False):
    ...
    Builds post training dataset by creating tuples that only contain weat words.
    """
    _, _, to_keep_words = apply_reduction(
        text, vocab, whitelist.copy(), min_freq)
...
with open(output_dir+"data_v3.pkl", "wb") as han:
    pickle.dump(data, han)
with open(output_dir+"vocab_v3.pkl", "wb") as han:
    pickle.dump(vocab, han)
with open(output_dir+"inv_vocab_v3.pkl", "wb") as han:
    pickle.dump(inv_vocab, han)
with open(output_dir+"unigram_counts_v3.pkl", "wb") as han:
...
tuple_set_1 = [x for x in tuple_set if x[0] in list_approx_words] 
