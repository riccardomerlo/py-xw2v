...
output_dir = 'gensim_shuffle_default_sentmin/'

syn0 = np.load(output_dir+'syn0_final_torch.npy')
syn1 = np.load(output_dir+'syn1_final_torch.npy')

with open(output_dir+"data_v3.pkl", "rb") as f:
    data = pickle.load(f)

with open(output_dir+"vocab_v3.pkl", "rb") as v:
  vocab = pickle.load(v)

with open(output_dir+"unigram_counts_v3.pkl", "rb") as u:
  unigram_counts = pickle.load(u)

with open(output_dir+"inv_vocab_v3.pkl", "rb") as iv:
...
# df = pd.DataFrame(columns=['sent_id', 'tuples_post', 'effect_size_pert_posttrain'])
...
# df.to_csv("effect_size_gensim.csv", index=False)
