...
from pytorch.dataset_torch import create_skipgram, read_corpus, split_given_size
from pytorch.model_torch import Word2VecModel
from pytorch.after_training_torch import _full_loss_torch, _true_loss_torch, _negative_sampling_loss_torch, post_training, post_training_optimized, get_sim_matrix, get_sim_perturbed, get_emb_og, get_emb_pert, compute_corr, get_variation_sim_matrix, effect_size, get_perturbed_emb_sent
...
print('number of tuples with batches: ', len(data_post))
...
for step, training_point in enumerate(data_post):
  ...
  for word in list_index_weat:
    if word in inputs and first==True: # keep a batch if it contains at least one weat word 
      ...
      first=False  
print("Tot. number of inputs to consider with batches: ", len(list_onlyweat))
...
random.seed(0)
text_ids = random.sample(range(len(text)), 30)
